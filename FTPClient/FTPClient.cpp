...
void FTPClient::ExploreRecursively(std::string path,std::list<std::string> &container, int depth)
{
	...
	{
		container.push_back(prefix+file);
		try
		{
...
std::vector<std::string> FTPClient::Split(std::string& s) const
{
	...
	std::string tok = "\r\n";
		...
		if(initialPos != s.length())
			strs.push_back(s.substr(initialPos, min(pos, s.length()) - initialPos + tok.length()));
